...
def test_day_halos_by_venue_and_category() -> None:
    ...
    assert day_halos([giants]) == ["giants"]
    assert day_halos([warriors]) == ["warriors"]
    assert day_halos([concert]) == ["concert"]


def test_day_halos_ordered_innermost_first() -> None:
    """CSS draws the rings in list order, so the sequence is load-bearing."""
    concert = _ev("Band", "Oracle Park", "2026-05-05T03:00:00+00:00", "concert", "1")
    warriors = _ev("Warriors", "Chase Center", "2026-05-05T02:30:00+00:00", "sports", "2")
    giants = _ev("Giants", "Oracle Park", "2026-05-05T02:05:00+00:00", "sports", "3")
    # Input order shuffled; output order must not be.
    assert day_halos([concert, warriors, giants]) == ["giants", "warriors", "concert"]


def test_concert_at_oracle_park_is_not_a_giants_halo() -> None:
    concert = _ev("Band", "Oracle Park", "2026-05-05T03:00:00+00:00", "concert")
    assert day_halos([concert]) == ["concert"]
...
def test_month_view_places_events_on_pacific_local_date() -> None:
    ...
    assert cells[date(2026, 5, 4)].halos == ["giants"]
...
def test_month_view_adjacent_month_days_keep_their_events() -> None:
    ...
    assert cell.halos == ["warriors"]


def test_month_view_sorts_a_days_events_by_start() -> None:
    late = _ev("late", "Chase Center", "2026-05-05T03:30:00+00:00", "concert", "1")
    early = _ev("early", "Oracle Park", "2026-05-05T02:05:00+00:00", "sports", "2")
